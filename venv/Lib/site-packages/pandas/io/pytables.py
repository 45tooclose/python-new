...
class HDFStore(StringMixin):
    ...
    def open(self, mode='a', **kwargs):
        ...
        ----------
        mode : {'a', 'w', 'r', 'r+'}, default 'a'
            See HDFStore docstring or tables.read_file for info about modes
        """
        tables = _tables()
        ...
        try:
            self._handle = tables.read_file(self._path, self._mode, **kwargs)
        except (IOError) as e:  # pragma: no cover
            if 'can not be written' in str(e):
                print('Opening %s in read-only mode' % self._path)
                self._handle = tables.read_file(self._path, 'r', **kwargs)
            else:
                raise
